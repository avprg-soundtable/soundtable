    ...
    pureDataOSCController(new PureDataOSCController)
{
    ...
    for(int n = 0; n < 15; n++){    //create first controlldatabuffer that should be overwritten by sending first signals
        QVector<float> audioObjectBuffer(19);
        for(int m = 0; m < 19; m++){
...
void SoundProcessor::process(float sequenzer, float masterVol,float seqSpeed, QVector< QVector<float> > rawData){
    ...
    int numberParams = 19;    //number of parameters
        ...
        if (rawData.isEmpty()==false){                            //
            if (rawData[i].isEmpty()==false){
                ...
                if(rawObject.isEmpty()==false){
                    ...
                    audioObject.append(calcSimpleFmOn(rawObject));
                    audioObject.append(calcSimpleFmNote(rawObject));
                    audioObject.append(calcSimpleFmVolume(rawObject));
                    audioObject.append(calcSimpleFmModul(rawObject));
                 }
             }
        }
...
float SoundProcessor::calcSawModul(QVector<float> rawObject){
    return rawObject[4]/rawObject[3]*12;
}
...
float SoundProcessor::calcSimpleFmOn(QVector<float> rawObject){
    // Number of corners
    if (rawObject[5]<=8){
        return 1;
    }else{
...
float SoundProcessor::calcSimpleFmNote(QVector<float> rawObject){
    float note=rawObject[3];
    int min = 64;
    int max = 45064;
    if(note<=min){
...
float SoundProcessor::calcSimpleFmVolume(QVector<float> rawObject){
    float vol;
    int min = 0;
    int max = 480;
    vol=rawObject[2];
...
float SoundProcessor::calcSimpleFmModul(QVector<float> rawObject){
   return rawObject[4]/rawObject[3]*12;
}
...
void SoundProcessor::send(QVector<float> controlData, QVector< QVector<float> > audioData){
    ...
    if (controlDataBuffer[1]!=controlData[1]){
     ...
     pureDataOSCController->sendOSC("/stopSeq",controlData[1]); //stop sequenezr counting
    }
    if (controlDataBuffer[2]!=controlData[2]){
     pureDataOSCController->sendOSC("/sequenz",controlData[2]);
     pureDataOSCController->sendOSC("/startSeq",controlData[2]); //start sequenezr counting
    ...
    if (controlDataBuffer[4]!=controlData[4]){
     ...
     for(int i = 0; i < audioData.size(); i++){
         ...
         if (audioObject[1]!=audioObjectBuffer[1]){
             ...
             if (audioObject[15]!=audioObjectBuffer[15]){
             pureDataOSCController->sendOSC(route+"/simpleFMOn",audioObject[15]);
         }
             if (audioObject[16]!=audioObjectBuffer[16]){
             pureDataOSCController->sendOSC(route+"/simpleFMNote",audioObject[16]);
         }
             if (audioObject[17]!=audioObjectBuffer[17]){
             pureDataOSCController->sendOSC(route+"/simpleFMVolume",audioObject[17]);
         }
             if (audioObject[18]!=audioObjectBuffer[18]){
             pureDataOSCController->sendOSC(route+"/simpleFMModul",audioObject[18]);
         }
